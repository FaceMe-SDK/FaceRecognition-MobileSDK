...
import com.fm.face.FeatureExtractor
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var ret = Activator.setActivation(assets)
        if(ret == Activator.ACTIVATE_SUCCESS) {
            FaceDetector.init(assets)
            LiveDetector.init(assets)
        } else {
            if(ret == Activator.ACTIVATE_APPID_ERROR) {
                textViewMessage.text = "AppID Error!"
            } else if(ret == Activator.ACTIVATE_INVALID_LICENSE) {
                textViewMessage.text = "Invalid License!"
            } else if(ret == Activator.ACTIVATE_LICENSE_EXPIRED) {
                textViewMessage.text = "License Expired!"
            }
        ...
        frameAnalyser = FrameAnalyser( this , boundingBoxOverlay, viewBackgroundOfMessage, textViewMessage )
